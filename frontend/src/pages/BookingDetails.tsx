                        ...
                        <div className="bg-white rounded-[2.5rem] p-8 shadow-sm border border-gray-100 relative overflow-hidden">
                            ...
                            <div className="relative pl-8 space-y-12">
                                ...
                                <div className="relative">
                                    ...
                                    <p className="text-xl font-black text-gray-900">{booking.tickets?.[0]?.trip?.route?.split('→')[0].trim() || 'Port de départ'}</p>
                                ...
                                <div className="relative">
                                    <div className="absolute -left-[27px] top-0 w-4 h-4 rounded-full bg-white border-4 border-gray-200 shadow-sm"></div>
                                    <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-1">Arrivée</p>
                                    <p className="text-xl font-black text-gray-900">{booking.tickets?.[0]?.trip?.route?.split('→')[1].trim() || 'Port d\'arrivée'}</p>
                                    <p className="text-sm font-bold text-gray-400 mt-1 uppercase tracking-tighter">Traversée Maritime</p>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="grid grid-cols-1 gap-8">
                            {booking.tickets?.map((ticket: any, idx: number) => (
                                <div key={idx} className="group overflow-hidden">
                                    {/* Design Billet Maritime Premium */}
                                    <div className="bg-white rounded-[3rem] border border-gray-100 shadow-md flex flex-col md:flex-row relative">
                                        {/* Décoration sur le côté */}
                                        <div className="absolute left-0 top-0 bottom-0 w-2 bg-ocean-600 transition-all group-hover:w-3"></div>

                                        {/* Partie Gauche: Infos Passager & Trip */}
                                        <div className="flex-1 p-10">
                                            <div className="flex items-center justify-between mb-8">
                                                <div className="flex items-center gap-4">
                                                    <div className="w-14 h-14 bg-ocean-50 rounded-2xl flex items-center justify-center text-ocean-600 transition-transform group-hover:scale-110">
                                                        <IdentificationIcon className="w-8 h-8" />
                                                    </div>
                                                    <div>
                                                        <p className="text-[10px] font-black text-gray-400 uppercase tracking-[0.2em] mb-1">Passager {idx + 1}</p>
                                                        <h4 className="text-2xl font-black text-gray-900 truncate max-w-[250px]">{ticket.passenger_name}</h4>
                                                    </div>
                                                </div>
                                            ...
                                            <div className="grid grid-cols-2 md:grid-cols-3 gap-y-8 gap-x-4 pt-8 border-t border-dashed border-gray-100">
                                                <div>
                                                    ...
                                                    <p className="text-sm font-black text-gray-800">{ticket.passenger_type === 'adult' ? 'ADULTE' : 'ENFANT'}</p>
                                                </div>
                                                <div>
                                                    <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-2">Catégorie</p>
                                                    <p className="text-sm font-black text-gray-800 uppercase">{ticket.nationality_group === 'african' ? 'Afrique' : ticket.nationality_group === 'senegalese' ? 'Sénégal' : 'International'}</p>
                                                </div>
                                                <div className="hidden md:block">
                                                    <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-2">Navire</p>
                                                    <p className="text-sm font-black text-ocean-600">{ticket.trip.ship}</p>
                                                </div>
                                                <div className="col-span-2 md:col-span-3">
                                                    <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-2">Traversée</p>
                                                    <div className="flex items-center gap-2 text-lg font-black text-gray-900">
                                                        <span>{ticket.trip.route.split('→')[0]}</span>
                                                        <span className="text-gray-300 font-normal">→</span>
                                                        <span>{ticket.trip.route.split('→')[1]}</span>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>

                                        {/* Partie Droite: QR Code (Le Coupon) */}
                                        <div className="w-full md:w-64 bg-gray-50/80 p-10 flex flex-col items-center justify-center border-t md:border-t-0 md:border-l-2 border-dashed border-gray-200 relative overflow-hidden">
                                            {/* Semicircles for ticket punch effect */}
                                            <div className="absolute top-1/2 -left-4 w-8 h-8 rounded-full bg-[#F8FAFC] hidden md:block -mt-4 shadow-inner"></div>
                                            <div className="absolute top-1/2 -right-4 w-8 h-8 rounded-full bg-[#F8FAFC] hidden md:block -mt-4 shadow-inner"></div>

                                            <div className="bg-white p-4 rounded-3xl shadow-sm border border-gray-100 mb-6 transform group-hover:scale-105 transition-all">
                                                <QRCode
                                                    value={ticket.qr_code_data || booking.booking_reference}
                                                    size={120}
                                                    style={{ height: "auto", maxWidth: "100%", width: "100%" }}
                                                />
                                            </div>
                                            <p className="text-[10px] font-black text-ocean-600 uppercase tracking-widest mb-1">Code de Voyage</p>
                                            <p className="text-[9px] font-bold text-gray-400 uppercase tracking-tighter text-center">Scanner pour embarquer</p>
                                        </div>
                                    </div>
                                </div>
                            ))}
