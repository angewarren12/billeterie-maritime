...
export default function ClientBookingDetail() {
    ...
    const handleDownloadTicket = async () => {
        try {
            ...
            const url = window.URL.createObjectURL(new Blob([blob as unknown as Blob]));
    ...
    if (!booking) return (
        <div className="text-center py-20">
            ...
            <Link to="/mon-compte" className="text-ocean-500 font-bold mt-4 block">Retour au dashboard</Link>
        </div>
    );
    ...
    return (
        <div className="container mx-auto px-4 py-8 max-w-7xl animate-in fade-in duration-700 pb-20">
            {/* Header / Nav */}
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-8">
                <Link to="/mon-compte" className="inline-flex items-center gap-2 text-gray-500 hover:text-ocean-600 dark:hover:text-white transition-colors font-bold group">
                            ...
                            <div className="space-y-6">
                                ...
                                <div className="pt-4 border-t border-white/10">
                                    <div className="flex justify-between items-center text-sm">
                                        <div>
                                            <p className="text-[10px] text-ocean-400 font-black uppercase tracking-widest">Départ</p>
                                            <p className="font-bold">{booking.tickets[0].trip.route?.departure_port?.name}</p>
                                        </div>
                                        <div className="text-ocean-500">
                                            <ArrowsRightLeftIcon className="w-4 h-4" />
                                        </div>
                                        <div className="text-right">
                                            <p className="text-[10px] text-ocean-400 font-black uppercase tracking-widest">Arrivée</p>
                                            <p className="font-bold">{booking.tickets[0].trip.route?.arrival_port?.name}</p>
                                        </div>
                                    </div>
                                </div>
                            </div>
